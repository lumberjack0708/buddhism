...
class Section {
    ...
    public function search($keyword) {
        ...
        // 搜尋小節內容
        $sectionSql = "SELECT 
		sec.id, sec.chapter_id, sec.title, sec.theme, 
        sec.outline, sec.key_points, sec.transcript, 
        NULL as verbatim_transcript, sec.youtube_id, 
        sec.order_index,c.name as chapter_name, 
        ...
        // 搜尋主題內容
        $themeSql = "SELECT 
        sec.id, sec.chapter_id, sec.title, 
        t.name as theme, t.outline, t.key_points, 
        t.transcript, t.verbatim_transcript, 
        t.youtube_id, sec.order_index,
        c.name as chapter_name, s.id as scripture_id, 
        s.name as scripture_name,'theme' as source_type
        FROM themes t
        ...
        return DB::select($sql, array(
            ...
            $searchTerm, $searchTerm, $searchTerm, $searchTerm, $searchTerm  // theme 參數
        ));
    }
